...
export function TextBlurIn({
  children, className, duration = 0.8, delay = 0, by = "word",
  staggerDelay = 0.04, as = "p", ...props
}: TextBlurInProps) {
    ...
    <Tag className={cn(className)} {...props}
      ...
      {units.map((unit, key) => {
        ...
        return (
          <motion.span key={key}
            variants={{
              hidden: { opacity: reduced ? 1 : 0, filter: reduced ? "blur(0px)" : "blur(10px)" },
              visible: { opacity: 1, filter: "blur(0px)" }
            }}
            transition={{ duration: reduced ? 0 : duration, delay: reduced ? 0 : delay + wordIndex * staggerDelay }}
